...
bool RegisterLine::MergeRegisters(const RegisterLine* incoming_line) {
  ...
  if (monitors_.size() != incoming_line->monitors_.size()) {
    verifier_->Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "mismatched stack depths (depth="        << MonitorStackDepth() << ", incoming depth=" << incoming_line->MonitorStackDepth() << ")";
    ...
    for (uint32_t idx = 0; idx < num_regs_; idx++) {
      ...
      if (depths != incoming_depths) {
        if (depths == 0 || incoming_depths == 0) {
          ...
          verifier_->Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "mismatched stack depths for register v" << idx                                                       << ": " << depths  << " != " << incoming_depths;
          break;
        }
      }
    }
